...
- 产品代码已就位（workspace：`crates/*` + `tools/*`）。**唯一规范来源是 `DECISIONS.md`**——动工前先读它（2026-08-26 起含 D23–D35 扩展性重构决策）；本文件只提炼其中最容易违反的硬约束。
...
- **焦点校验**：注入 Ctrl+V 前必须校验目标窗口存活且仍在前台，失败降级为仅复制。目标来源为常驻 `TargetTracker`（粘性热目标，见 D13），不再是「唤起面板时的一次性瞬时采样」。
- **一键上框是核心链路**（D13）：核心路径终点止于「素材进入输入框」，注入序列中**绝不出现 0x0D**；就绪度探测判定为 NotReady 时禁止注入，`Unknown` 时可注入但必须标 `verified:false`。
