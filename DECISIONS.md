...
### D76.5 点位编辑改弹窗 + 「要重启」误判定案（2026-09-06，用户验收反馈）

- **弹窗化**：D76.4 的内联编辑区在选中目标后没有回退出口（用户反馈）。设置面板区收缩为纯 FlatButton 列表，点目标弹独立弹窗（同重命名弹窗模式：即时挂载、顶部锚定 y=150、挡板拦背景、Esc 链先于设置面板收）。出口三路：取消 / 保存成功自动收 / Esc；保存失败内联报错弹窗不收，恢复默认回填留弹窗。设置面板任意关闭点（齿轮 toggle、关闭按钮）连带收弹窗。
- **「要重启才生效」判定为误解，热重载真机实锤**：用户真机日志复盘——安装版 settings.toml（mtime 10:26:42，`qianniu = 8 / WINDOW_HEIGHT-8`）所在会话 10:25:07 启动，**10:26:02 起的粘贴 ClickEvidence 已是新点位**（`x_logical: 8`，且窗口改高后 `y_logical` 随 `WINDOW_HEIGHT` 自动 926），10:27 的重启发生在生效之后。粘贴路径 `Vm::paste` 每次现查 `self.profiles`，无缓存计划。
...
### D78 自动以管理员运行（启动交棒 + 设置项默认关）（2026-09-06，用户提案）

- **动机**：D77 提示文案给了两条出路，「以管理员运行本应用」此前只能去右键菜单找入口。用户提案「应用打开时自动请求管理员权限」。落地为设置项「自动以管理员运行」（高级区，**默认关**）：开启后每次启动未提权时经 UAC 交棒重启；设置面板翻转开启且当前未提权时**立即交棒**（等价应用内一键「以管理员重启」，不必等下次启动）。
- **manifest 方案否决**：`requireAdministrator` 让标准用户根本无法启动、UAC 无条件每次弹、且不可按用户关闭；`highestAvailable` 同样不可关。设置项 + `ShellExecuteW runas` 交棒 = 用户显式选择、可随时关、UAC 只在未提权启动时出现。防环：交棒时命令行追加 `--elevation-relaunched` 标记，新进程见到标记不再交棒（即使异常环境下仍被判未提权也不会循环重启）。
- **默认关的理由**：提权对核心注入链路只解决一个边缘场景（目标以管理员运行），代价却是全体用户承担——每次启动弹 UAC，且**以管理员运行时 Explorer 拖入导入被 UIPI 硬拦**（导入走 OLE `IDropTarget`（D49），跨完整性级别的 OLE 拖放无解；`ChangeWindowMessageFilter` 救法只适用 WM_DROPFILES 消息型拖放，对我们无效），提权用户只能用导入按钮。默认关 + 一键开启把选择权留给真正遇到提权目标的用户。
- **提权判定用完整性 RID 不用 TokenElevation**：`current_process_is_elevated` = 自身令牌完整性 RID ≥ 高（0x3000）。UAC 关闭时管理员进程拿的就是完整令牌，TokenElevation 未必为真；「能不能注入」只由完整性级别决定（与 D77 的目标侧判定同一把尺子）。**2026-09-06 真机钉正**：初版误把高档写成 0x4000（实为 System 档），管理员进程 rid=0x3000 判定恒 false，提权分支从不触发——真机状态探针抓到后已改（D79 真机验证轮）。
...
**M1-b 批2 落地（2026-09-14，app-ui 同步接线 + 接收侧发现面 + 共享中角标）**：`SyncMessage::Request{asset_uuid}` 索取变体（serde `"kind":"request"`，validate 恒合法；worker 零改动——非 Pull 入站原样转发 UI、非 Pull 外发即发即收）。share_vm：`request_asset`（受信任册门，发 SYNC_SEND 索取行）、`direct_send`（按设备 id 直发，start_share 重构复用同一 SEND 行形态）、`PendingRequest` 队列 + `ShareAction::OpenRequestDialog`（先到先弹）。app-ui 接线：`ShareAction::PushSyncState`（READY 后推一次，控制面每次真实变更后重推 SYNC_STATE——引擎册整体替换，Pull 应答的唯一来源）；逐动作 Delta（`share_visibility_snapshot` 动作前取各设备可见集基线 + prev_rev，动作后 `delta_sync_lines` 逐接收方 diff 出 added/removed 组 Delta——**按 device_id 查表对齐而非按下标 zip**（动作期间可能 pair/unpair，两侧设备集不同构））；信任册 = 配对册（启动装载后 + pair/unpair 后刷新）；行构造收拢 ui-viewmodels（`sync_state_line`/`delta_sync_lines`，app-ui 依赖白名单不含 share/serde_json 纪律不破）。
- **Delta 触发点**：mark/revoke（共享态勾选）、拆域、解配（级联撤销推进 rev 的三处）；成员册调整不动 rev（无 Delta 可发，重推 SYNC_STATE 保 Pull 应答新鲜，接收方经 Pull/发现面打开全量补拉自愈——已知边缘，记档）；prev_rev == new_rev（幂等动作）不发。删除素材未级联 `revoke_asset` 的收口本批未接（已知边缘，后续批次补）。
- **发现面 UI**：顶栏「共享发现」IconButton（settings 左侧）→ 弹窗（对端行单选 + 选中对端可得清单逐行「索取」+ 刷新）→ 索取审批弹窗（peer 名 + resolver 解析素材名/大小）→ 「发送」= resolver.share_candidate → direct_send（M0 推送，双确认最后一环）；「拒绝」= 静默出队。打开即 `refresh_peer_views` 全量补拉（用户显式动作驱动）。
- **瓦片「共享中」角标**：TileData `shared: bool`，build_rows 从 SHARE_CONTROL（pub(crate)）现查 shared uuid 集；渲染 = 发送结果点正下方紫点（位置+颜色双重区分，可获得性与单次传输结果语义不同）；刷新点 = 标记/撤销/拆域/解配后 `grid.sync()` 整体重排（build_rows 现查，重排即取新值）。
