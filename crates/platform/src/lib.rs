...
pub enum FocusStep {
    ...
    /// 无声明时该级别自动跳过。这是概念动作「把焦点放入目标应用的输入框」的
    /// **点击式机制**之一：与 `AnchorClick` 的差别只在定位模型——这里是用户可配
    /// 的窗口内任意一点（原点=客户区左上角，逻辑像素，变量
    /// WINDOW_WIDTH/WINDOW_HEIGHT 实时求值）。哪个机制对哪个目标有效是观测
    /// 事实（如拼多多实测无法以点击恢复焦点，其画像就不声明任何点击级）。
    InputPointClick,
    /// 读取原生 caret 并以 MSAA 语义确认编辑器，不执行任何动作。
    CaretSemantic,
}

/// 「点击式聚焦机制」的坐标参数：一个窗口内可点击的表达式点。
///
/// 概念边界：上层的语义是 [`FocusStep::InputPointClick`] 实现的「把焦点放入
/// 目标应用的输入框」；本类型只是该机制的定位参数。有效点位随目标、版式与
/// 设备而异（微信为窗口右下角内缩、千牛实测为输入框内部、拼多多无有效点击
/// 区），因此建模为画像数据而非代码分支——设备差异由用户改写表达式自调。
///
/// 坐标系：客户区，原点=左上角，96-DPI 逻辑像素（点击时按目标窗口实时 DPI
/// 换算物理像素）。表达式支持整数四则运算、括号与两个实时变量：
/// `WINDOW_WIDTH`/`WINDOW_HEIGHT` = 客户区逻辑宽高。常见写法：
